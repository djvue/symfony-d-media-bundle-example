...
class ExceptionListener
{
        ...
        try {
            do {
                if ($exception instanceof ValidatorException) {
                    $this->handleValidatorException($event, $exception);
    ...
    public function handleValidatorException(ExceptionEvent $event, ValidatorException $exception): void
    {
        $event->allowCustomResponseCode();
        $data = [
            'success' => false,
            'code' => BaseApiController::CODE_BAD_REQUEST,
            'message' => $exception->getMessage(),
            'data' => [
                'errors' => $exception->getErrors(),
            ],
        ];
        $response = new JsonResponse($this->json($data), JsonResponse::HTTP_BAD_REQUEST, [], true);
        $event->setResponse($response);
        $e = FlattenException::createFromThrowable($event->getThrowable());
        $this->logger->notice(sprintf(
            'Validator Exception %s: "%s" at %s line %s',
            $e->getClass(),
            $e->getMessage(),
            $e->getFile(),
            $e->getLine()
